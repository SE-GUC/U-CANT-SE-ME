...
class ViewMyFees extends Component {
    ...
    async componentDidMount() {
        ...
        const {data: fees} = await axios.get(`http://localhost:5000/api/investors/viewMyFees/${investorID}/`)
...
render(){
  ...
  (
      this.state.fees.map((fees) => (<ViewMyFeesItem key={fees.companyName} fees={fees} valid={true} />))):
