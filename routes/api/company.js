...
router.post("/joi", (req, res) => {
  ...
  for (let i = 0; i < companies.length; i++) {
    if (companies[i].companyName === companyName) flagCompanyName = true;
    if (
      companies[i].investorID === investorID &&
      companies[i].companyType === "SSC" &&
      companyType === "SSC"
    )
      flagValidSSC = true;
  }
...
router.put("/joi", (req, res) => {
  ...
  for (let i = 0; i < companies.length; i++)
    if (companies[i].companyName === companyName) companyExists = true;
  if (!companyExists)
    return res.status(404).send({ error: "ERROR 404: Company does not exist" });
  const schema = {
    companyName: Joi.string()
  ...
  if (newCompanyName !== undefined) {
    ...
    for (let i = 0; i < companies.length; i++)
      if (companies[i].companyName === newCompanyName) validCompanyName = false;
    if (!validCompanyName)
  ...
  if (companyStatus !== undefined)
    for (let i = 0; i < companies.length; i++)
      if (
        ...
        (companyNameGiven && companies[i].companyName === newCompanyName)
      )
